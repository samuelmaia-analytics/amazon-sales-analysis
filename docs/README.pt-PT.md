...
Este guia resume arquitetura, operação e padrão de engenharia para revisão técnica.
...
O repositório demonstra um fluxo analítico orientado a produção com:

- execução batch reprocessável
- validação de contrato, schema e qualidade
- controlo de regressão de KPIs entre runs
...
- artefactos imutáveis em `reports/runs/<run_id>/`
- snapshots `latest` estáveis para consumo
- retenção explícita de runs
- estado operacional preservado também em falhas
...
- testes com dados sintéticos por omissão
